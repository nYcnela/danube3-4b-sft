...
# -*- coding: utf-8 -*-
"""
H2O-Danube3-4B - SUPERVISED (SFT) Training with QLoRA

Model: h2oai/h2o-danube3-4b-chat (Instruct version)
Data:  ../model_v12.0/10_2training_prompts (shared with Mistral v12.0)
Output: outputs/model_danube_supervised/

Based on qlora_llama3_supervised.py
Danube is a smaller Mistral-like model (4B params) - faster training!
"""
...
DATA_ROOT = SCRIPT_DIR.parents[3] / "data" / "json" / "manual" / "training_prompts"
OUT_DIR = SCRIPT_DIR.parents[3] / "outputs" / "manual" / "danube_4b" / "model_danube_supervised"
...
# Training hyperparams - OPTIMIZED for 4B model (faster than 8B Llama)
NUM_EPOCHS = 3
BATCH_SIZE = 32  # 4B model allows larger batch
GRAD_ACC_STEPS = 2  # No accumulation needed with large batch
LR = 2e-5
WEIGHT_DECAY = 0.01
WARMUP_RATIO = 0.1
MAX_SEQ_LEN = 1536

# Danube uses same architecture as Mistral - same target modules
...
def main():
    print("=" * 70)
    print("H2O-DANUBE3-4B - SUPERVISED TRAINING (QLoRA)")
    print("=" * 70)
    ...
    model = load_model_for_training(MODEL_ID, use_4bit=(DEVICE.type == "cuda"))
